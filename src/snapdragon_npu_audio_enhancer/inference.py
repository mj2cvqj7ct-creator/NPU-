...
class OnnxRuntimeInferenceBackend:
    ...
    def infer(self, features: AudioFeatures) -> EnhancementControls:
        ...
        if controls.size < 8:
            raise ValueError("Enhancement model must output eight DSP controls.")
        return EnhancementControls(
            ...
            compressor_threshold_db=float(np.clip(controls[5], -36.0, -6.0)),
            compressor_ratio=float(np.clip(controls[6], 1.0, 6.0)),
            limiter_ceiling_db=float(np.clip(controls[7], -6.0, -0.1)),
        )
