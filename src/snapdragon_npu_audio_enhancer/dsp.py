...
class FeatureExtractor:
    ...
    def extract(self, frame: AudioFrame) -> AudioFeatures:
        ...
        crest_factor = linear_to_db(peak) - linear_to_db(rms)
        transient_density = _transient_density(mono)
...
class RuleBasedEnhancer:
    ...
    def derive_controls(self, features: AudioFeatures) -> EnhancementControls:
        ...
        transient_gain = np.clip((10.0 - features.crest_factor_db) * 0.18, -0.8, 1.2)
        if features.clipping_ratio > 0.001 or features.peak_db > -0.2:
            ...
            transient_gain = min(transient_gain, 0.0)
        return EnhancementControls(
            ...
            transient_gain_db=float(transient_gain),
        )
    ...
    def process(self, frame: AudioFrame, controls: EnhancementControls) -> AudioFrame:
        ...
        samples = _restore_transients(samples, controls.transient_gain_db)
...
def merge_controls(base: EnhancementControls, overlay: EnhancementControls, weight: float) -> EnhancementControls:
    ...
    return EnhancementControls(
        ...
        transient_gain_db=base.transient_gain_db * inv + overlay.transient_gain_db * weight,
...
def _transient_density(mono: np.ndarray) -> float:
    if len(mono) < 2:
        return 0.0
    diff = np.abs(np.diff(mono))
    threshold = max(float(np.percentile(diff, 92.0)), 1e-6)
...
def _restore_transients(samples: np.ndarray, gain_db: float) -> np.ndarray:
    gain = db_to_linear(float(np.clip(gain_db, -1.5, 2.0))) - 1.0
    if abs(gain) < 1e-6:
        return samples
    previous = np.vstack([samples[:1], samples[:-1]])
    transient = samples - previous
    restored = samples + transient * gain * 0.35
