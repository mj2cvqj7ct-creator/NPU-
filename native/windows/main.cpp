...
std::wstring BuildRealtimeStatus() {
    ...
    stream
        ...
        << L"DSP graph: peak normalization -> multiband focus -> vocal lift -> mid/side holographic stage -> soft limiter\r\n"
        << L"NPU target: Snapdragon X NPU\r\n"
        << L"Backend: ONNX Runtime QNN Execution Provider\r\n"
        << L"Audio route: Spotify/Apple Music/YouTube Music loopback -> NPU graph -> ASIO exclusive output\r\n"
        << L"Driver target: XMOS USB DAC Driver Control Panel\r\n"
        << L"Latency target: minimum stable ASIO buffer, target 32 samples, underrun guard 2 periods\r\n"
...
std::wstring BuildRecommendationStatus() {
    ...
    stream
        ...
        << L"Model: two-tower deep embedding ranker + session feedback -> ONNX/QNN NPU target\r\n"
