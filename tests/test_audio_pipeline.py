...
from npu_audio_enhancer.realtime import (
    LowLatencyBufferPlan,
    ServiceState,
...
class AudioPipelineTest(unittest.TestCase):
    ...
    def test_holographic_profile_increases_vocal_band_energy(self) -> None:
        audio = generate_demo_buffer(duration_seconds=0.08)

        balanced = enhance_audio(audio, get_profile("balanced"))
        holographic = enhance_audio(audio, get_profile("holographic-vocal-stage"))
    ...
    def test_realtime_status_mentions_npu_streaming_requirements(self) -> None:
        ...
        self.assertIn("0.667 ms", status)
        self.assertIn("dropout guard: 3 x 32-sample ring buffers", status)
...
def _band_delta_energy(samples: list[float]) -> float:
    previous = 0.0
    energy = 0.0
